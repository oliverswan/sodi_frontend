                    ...
                    <tr v-for="entry in filteredValuesSorted " track-by="entry" @click="rowClickHandler($event, entry)">
                        <td v-for="column in displayColsVisible" track-by="column"
                            ...
                            <span v-if="column.renderfunction!==false">
                                <row-action :entry="entry" columnname:="columnname"></row-action>
                            </span> <!-- 如果有展现函数 -->

                            <span v-else-if="!column.editable"> {{ entry[column.name] }} </span> <!-- 如果不可编辑 -->
...
<script>
    ...
    var rowAction = {
      template: 
      '<div class="btn-group" role="group"  ref="actiondiv" >'+
                        '  <button type="button" class="btn btn-sm btn-primary"><span class="glyphicon " @click="downloadFiles" aria-hidden="true">Manage Files</span></button>'+
    ...
    //       '</div>',
      ...
      methods: {
        downloadFiles: function () {
             this.$router.push({
                  name: 'downloadTrackingFiles',
                  params:this.entry
              });
                // this.$http.get(API.host+"/api/items/receive/"+this.entry.id).then(function(res){
                //             if(res.data){
                //             alert("success!");
                //             }
                //         },function(res){
